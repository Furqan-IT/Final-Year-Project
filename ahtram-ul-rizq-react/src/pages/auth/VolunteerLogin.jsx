...
const VolunteerLogin = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await postApiService(`/volunteer-login`, formData)
            console.log(response)
            // if response.data.success => show some toast notification, and redirect to /auth/login
            if (response.data.success && response.data.volunteer.formType === 'volunteer') {
                // Show toast notification for success
                toast.success('Volunteer User logged in successfully');
                // save it in browser local storage
                localStorage.setItem('authtoken', response.data.authToken)
                const storedAuthToken = localStorage.getItem('authtoken');
                if (storedAuthToken) {
                    dispatch(setAuthToken(storedAuthToken));
                    dispatch(setAdminRole(response.data.volunteer.formType))
                }
                setTimeout(() => {
        ...
        <>
            <div className='container bg-dark text-secondary'>
                <div className="my-4 py-8">
                    <h1 className='text-danger text-center'>Volunteer Login Form</h1>
